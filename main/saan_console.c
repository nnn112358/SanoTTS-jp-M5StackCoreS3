...
static bool port_init(void) {
    ...
    ESP_LOGI(TAG, "入力: USB Serial/JTAG（CoreS3 の USB-C）");
    return true;
}
...
menuconfig で UART か USB Serial/JTAG を選ぶこと"
#endif
 ...
 *
 * かつて本家の readline の中で `saan_line ln;` を毎回作って `saan_line_reset()` して
 * いたが、それだと **CRLF の「次の LF を吸う」フラグ (`cr`) が毎回消える**。
 * `\r` で行が完成 → 合成 → 次の読み取りで reset → 遅れて届いた `\n` が
